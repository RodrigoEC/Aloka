...
addFilme(Titulo, Diretor, Data, Genero, Estoque, Result) :-
    geraIdFilme(Id),
    getFilmeById(Id, Filme),
    (   not(member(_, Filme))
    ->  open('./dados/Filmes.csv', append, File),
        writeln(File,
                (Id, Titulo, Diretor, Data, Genero, Estoque)),
        close(File),
        Result=Id
    ;   addFilme(Titulo, Diretor, Data, Genero, Estoque, Result)
    ).

reescreveFilme(Id, Titulo, Diretor, Data, Genero, Estoque) :-
    open('./dados/Filmes.csv', append, File),
    writeln(File,
            (Id, Titulo, Diretor, Data, Genero, Estoque)),
    close(File).
...
geraArrayGenero(Genero, [H|T], Arr) :-
    (   elementByIndex(4, H, G),
        member(Genero, [G])
    ->  geraArrayGenero(Genero, T, Arr1),
        concatenar(Arr1, [H], Arr)
    ;   geraArrayGenero(Genero, T, Arr1),
        concatenar(Arr1, [], Arr)
    ).
